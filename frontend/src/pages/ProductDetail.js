    ...
    return (
      <>
        {product && (
          ...
          <div>
              <label htmlFor="size-select">سایز های موجود</label>
              <select id="size-select" value={selectedSize ? JSON.stringify(selectedSize) : JSON.stringify(product.sizes[defaultSize])} onChange={(e) => setSelectedSize(JSON.parse(e.target.value))}>
                {product?.sizes?.slice()?.sort((a,b) => a.size - b.size)?.map((size, index) => (
                  <option key={index} value={JSON.stringify(size)}>
                    AF {size?.size} - {formatPrice(size?.price)} {size.quantity <= 3 ? `(only ${size.quantity} left)`  : "" }
                  </option>
                ))}
              </select>
            </div>
            <div className='divider'>
              <button onClick={() => addToCart({ product: product, size: selectedSize?.size || product.sizes[0].size, price: selectedSize?.price || product.defaultPrice })}>افزودن به لیست خرید</button>
              <button className='second-button' onClick={() => toggleWishlistItem(product)}>
                <span>لیست دلخواه</span><FontAwesomeIcon icon={itemExists ? icons.heartFull : icons.heart}/>
              </button>
            </div>  
